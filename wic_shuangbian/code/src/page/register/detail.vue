<template>
  <div class="register">
    ...
    <div v-if="configInfo.session" class="welcome">
      欢迎注册{{ configInfo.session + configInfo.name + configInfo.system }}
    </div>
    ...
    <div class="form">
      <Form
        ...
        :model="form"
        :rules="ruleValidate"
        :style="{ marginLeft: current === 0 ? '-110px' : 'auto' }"
        :label-width="current === 0 ? 250 : 140"
      >
        <!-- 第一步 -->
        <div v-if="current == 0">
          ...
          <FormItem label="负责人证件类型" prop="legal_certificate_type">
            <RadioGroup
              v-model="form.legal_certificate_type"
          ...
          <FormItem label="主体资质证明" prop="file_prove">
            <div class="file_button">
              <img
                src="../../assets/icon_wjsc.png"
                class="logo"
                v-if="form.file_prove === ''"
              />
              <img src="../../assets/icon_sccg.png" class="logo" v-else />
              <span class="text">
                {{ form.file_prove !== "" ? "上传成功" : "上传文件" }}
              </span>
            </div>
            <FileUpLoad
              id="file_prove"
              :value="form.file_prove"
              :fileName="file.file_prove"
              tips="主体资质证明"
              @onUploadFile="onUploadFile"
            ></FileUpLoad>
            <div class="file_text">
              须提供主体资质证明（营业执照副本、组织机构代码证、事业单位法人证书等），支持格式：rar、zip、doc、docx、pdf，单个文件不能超过20MB。
            </div>
          </FormItem>
          <FormItem label="企业财务会计报告或资信证明" prop="file_financial">
            <div class="file_button">
              <img
                src="../../assets/icon_wjsc.png"
                class="logo"
                v-if="form.file_financial === ''"
              />
              <img src="../../assets/icon_sccg.png" class="logo" v-else />
              <span class="text">
                {{ form.file_financial !== "" ? "上传成功" : "上传文件" }}
              </span>
            </div>
            <FileUpLoad
              id="file_financial"
              :value="form.file_financial"
              :fileName="file.file_financial"
              tips="企业财务会计报告或资信证明"
              @onUploadFile="onUploadFile"
            ></FileUpLoad>
            <div class="file_tip">
              须提供2021至2022年度经第三方会计师事务所审计的企业财务会计报告或银行出具的资信证明，支持格式：rar、zip、doc、docx、pdf，单个文件不能超过20MB。
            </div>
          </FormItem>
          <FormItem label="纳税证明和社会保险证明" prop="file_tax">
            <div class="file_button">
              <img
                src="../../assets/icon_wjsc.png"
                class="logo"
              ...
              <img src="../../assets/icon_sccg.png" class="logo" v-else />
              <span class="text">
                {{ form.file_tax !== "" ? "上传成功" : "上传文件" }}
              </span>
            </div>
            <FileUpLoad
              id="file_tax"
              :value="form.file_tax"
              :fileName="file.file_tax"
              tips="纳税证明和社会保险证明"
              @onUploadFile="onUploadFile"
            ></FileUpLoad>
            <div class="file_tip">
              须提供2021年或2022年至少一个月的依法缴纳税收和社会保险费的相关证明材料，支持格式：rar、zip、doc、docx、pdf，单个文件不能超过20MB。
            </div>
          </FormItem>
          <FormItem
            label="书面声明函及失信被执行人查询记录"
            prop="file_declaration"
          >
            <div class="file_button">
              <img
                src="../../assets/icon_wjsc.png"
                class="logo"
                v-if="form.file_declaration === ''"
              />
              <img src="../../assets/icon_sccg.png" class="logo" v-else />
              <span class="text">
                {{ form.file_declaration !== "" ? "上传成功" : "上传文件" }}
              </span>
            </div>
            <FileUpLoad
              id="file_declaration"
              :value="form.file_declaration"
              :fileName="file.file_declaration"
              tips="书面声明函及失信被执行人查询记录"
              @onUploadFile="onUploadFile"
            ></FileUpLoad>
            <div class="file_tip">
              需提供在经营活动中没有重大违法记录，所销售、展示的商品及服务须不得侵犯第三方知识产权的书面声明函及中国执行信息公开网（zxgk.court.gov.cn）失信被执行人查询记录，支持格式：rar、zip、doc、docx、pdf，单个文件不能超过20MB。
            </div>
          </FormItem>
        ...
        <div v-if="current == 1">
          ...
          <FormItem :label-width="68" class="ivu_item">
            <Checkbox v-model="single"></Checkbox>
            我已阅读并同意
            <span
              class="agreement"
              @click="(agreement = true), (agreementstate = 1)"
            >
              《用户服务协议及隐私政策》
            </span>
            <span
              class="agreement"
              @click="(agreement = true), (agreementstate = 2)"
              style="margin-left: -16px"
            >
              《第六届世界智能大会双边会谈注册审核条件说明》
            </span>
          </FormItem>
          <FormItem :label-width="68">
            <Checkbox v-model="ensure"></Checkbox>
            申请人保证以上所提交的材料和信息均如实申报，不存在虚假材料和虚假陈述。
          </FormItem>
        </div>
      </Form>
      <div v-if="current == 2" class="loginSuccess">
        <div class="flex-box">
    ...
    <Modal v-model="agreement" width="1000">
      ...
      <div
        :style="{ height: agreementstate === 1 ? '700px' : 'auto' }"
        class="agreementContent"
      >
        <pre v-if="agreementstate === 1">
    <h3>用户服务协议</h3>
    世界智能大会组委会（下称“组委会”）在此特别提醒您（下称“用户”）认真阅读、充分理解本《软件许可及用户服务协议》（下称本协议），除非您接受本协议所有条款，否则您无权注册、登录或使用本协议所涉相关服务。您的注册、登录、使用等行为将视为对本协议的接受。
    您对本协议的接受即受本协议全部条款的约束，包括接受组委会对任意服务条款随时所做的任何修改。本协议可随时更新，更新后的协议条款一旦公布即代替原来的协议条款，恕不再另行通知，用户可在相关服务页面查阅最新版协议条款。如果您不接受修改后的条款，请立即停止使用本服务，用户继续使用将被视为已接受了修改后的协议。
一、协议适用主体范围
1.1 本协议是用户与组委会之间关于用户下载、安装、使用、复制软件（下称“本软件”），以及使用相关服务所订立的协议。
1.2 “用户”是指注册、登录、使用、浏览本服务的个人或组织。
1.3 本协议内容同时包括组委会可能不断发布的关于本服务的相关协议、业务规则等内容。上述内容一经正式发布，即为本协议不可分割的组成部分，您同样应当遵守。
二、关于本服务
2.1 本服务的形式
...
4.3 组委会对相关信息采用专业加密存储与传输方式，保障用户个人信息的安全。
组委会将运用各种安全技术和程序建立完善的管理制度来保护您的个人信息，以免遭受未经授权的访问、使用或披露。
4.4 组委会非常重视对未成年人个人信息的保护。若您是18周岁以下的未成年人，在使用服务前，应事先取得您家长或法定监护人的书面同意。
五、主权利义务条款
5.1 帐号使用规范
5.1.1 您在使用本服务前需要注册一个帐号。
5.1.2 本软件帐号的所有权归组委会所有，用户完成申请注册手续后，仅获得帐号的使用权，且该使用权仅属于初始申请注册人。
5.1.3 用户有责任妥善保管注册帐户信息及帐户密码的安全，用户需要对注册帐户以及密码下的行为承担法律责任。用户同意在任何情况下不向他人透露帐户及密码信息。当在您怀疑他人在使用您的帐号时，您应立即通知组委会。
5.1.4 用户注册本软件帐号后如果长期不登录该帐号，组委会有权回收该帐号，以免造成资源浪费，由此带来的任何损失均由用户自行承担。
5.2 用户注意事项
...
5.2.3.2 组委会有权根据实际情况自行决定单个用户在本软件及服务中数据的最长储存期限，并在服务器上为其分配数据最大存储空间等。您可根据自己的需要自行备份本软件及服务中的相关数据；
...
七、知识产权声明
7.1 组委会是本软件的知识产权权利人。本软件的一切著作权、商标权、专利权、商业秘密等知识产权，以及与本软件相关的所有信息内容（包括但不限于文字、图片、音频、视频、图表、界面设计、版面框架、有关数据或电子文档等）均受中华人民共和国法律法规和相应的国际条约保护，组委会享有上述知识产权，但相关权利人依照法律规定应享有的权利除外。
...
9.3 本协议自您同意并成功注册为本软件用户之日起生效，除非组委会终止本协议或者您丧失用户资格，否则本协议始终有效。本协议终止并不免除用户根据本协议或其他有关协议、规则所应承担的义务和责任。
...
五、未成年人保护
我们非常重视对未成年人个人信息的保护。根据相关法律法规的规定，若您是18周岁以下的未成年人，在使用本网站服务前，应事先取得您的家长或法定监护人的书面同意。若您是未成年人的监护人，当您对您所监护的未成年人的个人信息有相关疑问时，您可以通过来电进行咨询。
六、隐私政策的更新
我们可能适时修订本隐私政策的条款，该修订构成本隐私政策的一部分。如可能造成您在本隐私政策下权利的实质减少或扩大收集、使用信息的范围等重要规则变更时，我们将在修订生效前通过在主页上显著位置提示通知您。在该种情况下，若您继续使用我们的服务，即表示同意受经修订的本隐私政策的约束。
七、联系我们
如您对本政策及其他相关事宜有疑问，您可以通过来电进行咨询。我们将在1-3个工作日内审核所涉问题，并在验证您的用户身份后予以回复。
        </pre>
        <pre v-if="agreementstate === 2">
    为保证大会组织的双边会谈顺利进行，防控相关法律风险，参加注册的企业资格审核需满足如下条件：
    一、具有独立承担民事责任能力的法人或其他组织。须提供主体资质证明（营业执照副本、组织机构代码证、事业单位法人证书等）；
    二、具有良好的商业信誉和健全的财务会计制度。须提供2021至2022年度经第三方会计师事务所审计的企业财务会计报告或银行出具的资信证明；
    三、有依法缴纳税收和社会保障资金的良好记录。须提供2021年或2022年至少一个月的依法缴纳税收和社会保险费的相关证明材料；
    四、在经营活动中没有重大违法记录，所销售、展示的商品及服务须不得侵犯第三方知识产权，须提供书面声明函及中国执行信息公开网（zxgk.court.gov.cn）失信被执行人查询记录。
         </pre
        >
      </div>
      <div slot="footer"></div>
...
import Load from 'huyue-tools-load'
export default {
  name: "basicDetail",
  components: {
    ImgUpLoad,
        ...
        legal_certificate_positive: [
          {
            ...
            validator: (rule, value, callback) => {
              ...
              if (!this.form.legal_certificate_positive && this.current === 0) {
        ...
        name: [
          {
            ...
            trigger: "blur,change",
            validator: (rule, value, callback) => {
              if (!this.form.name && this.current === 0) {
                callback(new Error("请输入企业名称"));
              } else if (this.form.name.length < 1 && this.current === 0) {
                callback(new Error("请输入企业名称"));
              } else {
                callback();
              }
        ...
        register_no: [
          {
            ...
            validator: (rule, value, callback) => {
              ...
              if (!this.form.register_no && this.current === 0) {
                callback(new Error("请输入企业统一社会信用代码"));
              } else {
                if (reg.test(value) && this.current === 0) {
                  callback();
                } else {
                  return callback(
        ...
        legal_contacts: [
          {
            ...
            trigger: "blur,change",
            validator: (rule, value, callback) => {
              if (!this.form.legal_contacts && this.current === 0) {
        ...
        legal_certificate_type: [
          {
            ...
            trigger: "blur,change",
            validator: (rule, value, callback) => {
              if (!this.form.legal_certificate_type && this.current === 0) {
        ...
        legal_certificate_code: [
          {
            ...
            trigger: "blur,change",
            validator: (rule, value, callback) => {
              if (!this.form.legal_certificate_code && this.current === 0) {
        ...
        phone: [
          {
            ...
            validator: (rule, value, callback) => {
              const reg = /^1[3-9]\d{9}$/;
              if (!this.form.phone && this.current === 1) {
        ...
        short_message: [
          {
            ...
            trigger: "blur,change",
            validator: (rule, value, callback) => {
              if (!this.form.short_message && this.current === 1) {
        ...
        contact_name: [
          {
            ...
            trigger: "blur,change",
            validator: (rule, value, callback) => {
              if (!this.form.contact_name && this.current === 0) {
        ...
        business_type: [
          {
            required: true,
            validator: (rule, value, callback) => {
              if (!this.form.business_type[0] && this.current === 0) {
        ...
        user_name: [
          {
            ...
            trigger: "blur,change",
            validator: (rule, value, callback) => {
              if (!this.form.user_name && this.current === 1) {
        ...
        password: [
          {
            ...
            validator: (rule, value, callback) => {
              ...
              if (!this.form.password && this.current === 1) {
        ...
        confirm_password: [
          {
            ...
            trigger: "blur,change",
            validator: (rule, value, callback) => {
              if (!this.form.confirm_password && this.current === 1) {
        ...
        file_financial: [
          {
            ...
            validator: (rule, value, callback) => {
              if (value === "") {
                callback(new Error("请上传企业财务会计报告或资信证明"));
              } else {
                callback();
              }
            }
        ...
        file_tax: [
          {
            ...
            validator: (rule, value, callback) => {
              if (value === "") {
                callback(new Error("请上传纳税证明和社会保险证明"));
              } else {
                callback();
              }
            }
        ...
        file_declaration: [
          {
            ...
            validator: (rule, value, callback) => {
              if (value === "") {
                callback(new Error("请上传书面声明函及失信被执行人查询记录"));
              } else {
                callback();
              }
            }
  ...
  created() {
    this.current = Number(this.$route.query.status) || 0;
    apis
      .GetTimeRange()
      .then(res => {
        ...
        if (this.configInfo.session) {
          // document.title = `${this.configInfo.session}届世界智能大会双边会谈系统`;
          document.title =
  ...
  methods: {
    ...
    initCheckCode() {
      let that = this
      Load.js('https://ssl.captcha.qq.com/TCaptcha.js', function () {
        const dom = document.getElementById('check-code')
        //2076719957 滑块 APPid
        that.txCode = new TencentCaptcha(dom, '2076719957', (res) => {
          console.log(res)
          if (res.ret !== 0) {
            return
          }
    ...
    onUploadFile(url, id, name) {
      console.log(url, id, name, "++++");
      if (id === "file_prove") {
        this.file.file_prove = name;
        this.form.file_prove_name = name;
        this.form.file_prove = url;
      } else if (id === "file_financial") {
        this.file.file_financial = name;
        this.form.file_financial_name = name;
        this.form.file_financial = url;
      } else if (id === "file_tax") {
        this.file.file_tax = name;
        this.form.file_tax_name = name;
        this.form.file_tax = url;
      } else if (id === "file_declaration") {
        this.file.file_declaration = name;
        this.form.file_declaration_name = name;
        this.form.file_declaration = url;
      }
    },
    // 表单提交
    handleSubmit() {
      if (this.single && this.ensure) {
        ...
        if (!this.single) {
          this.$Message.warning(
            "请勾选“我已阅读并同意《用户服务协议及隐私政策》《第六届世界智能大会双边会谈注册审核条件说明》”"
          );
        } else if (!this.ensure) {
          this.$Message.warning(
            "请勾选“申请人保证以上所提交的材料和信息均如实申报，不存在虚假材料和虚假陈述。”"
          );
        }
      }
    },
    ...
    changeType(e) {
      this.$refs.form.validateField("legal_certificate_code");
    }
  },
  beforeDestroy() {
