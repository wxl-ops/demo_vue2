...
// import * as GUI from "dat.gui";//这是完全体
import { GUI } from "dat.gui"; //这是部分引入
export default {
  name: "App",
  components: {},
  mounted() {
    ...
    // 通过gui创建分类菜单
    const matFolder = gui.addFolder("光照");
    //创建一个对象，对象属性的值可以被GUI库创建的交互界面改变
    // gui增加交互界面，用来改变obj对应属性
    matFolder.add(ambient, "intensity", 0, 2.0).name("环境光强度"); //改变环境光照强度,当第三位和第四位是数值类型时,传入参数分别为  (控制对象,对象具体属性,最小值，最大值)
    matFolder
      .add(directionalLight, "intensity", 0, 2.0)
      .name("平行光强度")
      .step(0.1); //step设置步长，就是每次更改的单位值
    const colorFolder = gui.addFolder("颜色");
    // 生成颜色改变的交互界面,当通过gui更改某个值时,onChange方法就会被调用
    colorFolder
      .addColor(obj, "color")
      .name("修改颜色")
      .onChange((value) => {
    ...
    //传入参数为数组时
    moveFolder.add(mesh.position, "x", [-150, 0, 150]).onChange((value) => {
      mesh.position.y = value;
    });
    //传入参数为对象时
    moveFolder
      .add(mesh.position, "x", { left: -150, center: 0, right: 150 })
      .onChange((value) => {
        mesh.position.z = value;
      });
    const animationFolder = gui.addFolder("执行动画");
    //传入参数为布尔值时
    animationFolder.add(obj, "bool").name("旋转动画");
    // gui.add(mesh.position, "y", -150, 150);
    // gui.add(mesh.position, "z", -150, 150);
  },
  // // 阵列模型
  // mounted() {
  //   //创建3D场景
